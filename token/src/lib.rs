...
impl Wallet {
	...
	pub fn request(&self, index: DerivationIndex) -> PublicKey {
	...
	pub fn receive(&self, amount: U256, spend: PrivateKey, index: DerivationIndex) {
	...
	pub fn find(&self, request: PrivateKey) -> DerivationIndex {
...
impl<'de> Deserialize<'de> for Wallet {
	...
	where
		D: Deserializer<'de>,
	{
		// do not deserialize SecretKey
		self.1.deserialize(deserializer)
	}
}
...
impl WalletExt for Client {
	pub fn act_wallet_get(sk: SecretKey) -> Result<Wallet, String> {
		// TODO
	}
}
...
impl ActExt for Client {
	...
	pub async fn act_create(
		...
		let (_paid, token_info_address) = self.chunk_put(&token_info, payment)
		...
		let (_paid, genesis_address) = self.graph_entry_put(genesis, payment)
			.await.map_err(|e| format!("{}", e))?;
...
//		let sk = SecretKey::random();
		...
		wallet.receive(genesis_spend, issuer_index);
		println!("Wallet: {:?}", wallet);
		...
		wallet.receive(*spend_address.owner(), rest_index, rest_amount);
		wallet.receive(*spend_address.owner(), receive_index, receive_amount);
		println!("Wallet: {:?}", wallet);
	...
	fn pubkey_from_derived_sk_equals_derived_pubkey() {
		...
		let pubkey_from_derived_sk = sk.derive_child(&index).public_key();
		println!("pubkey_from_derived_sk: {:?}", pubkey_from_derived_sk);
		let derived_pubkey = pk.derive_child(&index);
