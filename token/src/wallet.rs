...
use autonomi::{Client, SecretKey, PublicKey, XorName, ScratchpadAddress, Bytes,
...
impl Wallet {
	...
	pub fn receive(&mut self, amount: U256, received_token_id: XorName, spend: PublicKey) -> Result<(), String> {
	...
	pub fn pk_of_token(&self, token_id: XorName) -> Option<PublicKey> {
		self.index_of_token(token_id).map(|index| self.0.derive_child(&index.to_be_bytes::<32>()))
	}
	...
	pub fn received_spend(&self, token_id: XorName, spend: PublicKey) -> bool {
		self.1.get(&Some(token_id)).and_then(|(_index, spends)| {
			match spends.iter()
...
impl WalletExt for Client {
	...
	async fn act_wallet_save(&mut self, wallet: &Wallet, sk: &SecretKey, payment: &PaymentOption) -> Result<PublicKey, String> {
		...
		match existing {
			Some(_) => {
				self.scratchpad_update(sk, 0, &Bytes::from(data)).await
					.map(|_| sk.public_key())
			},
			None => {
				self.scratchpad_create(sk, 0, &Bytes::from(data), payment.clone()).await
...
mod tests {
	...
	fn de_ser_ialize() -> Result<(), String> {
		...
		let mut w = Wallet::new(SecretKey::from_hex(
			"4f7eedb7b093537a4402daa0769dfca018520ee3ea2107338d89cbfcc312451b"
		).map_err(|e| format!("{e}"))?.public_key());
